...
In order to prevent this first type of pathology from occuring, we define $P$ such that a dangling node has equal probability of transitioning to any random node, analogous to how a web surfer can type a hyperlink directly to a webpage.
...
Note that now, $P$ is column-stochastic by construction. Below is the Python code used to construct $P$ from an adjacency list representation of the graph of webpages.
